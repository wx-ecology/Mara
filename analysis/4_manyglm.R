...
#  saveRDS(anova.adj.I, file = "./results/manyglm_anova_I.rds")
...
# saveRDS(anova.adj.S, file = "./results/manyglm_anova_S.rds")
...
# saveRDS(anova.adj.R, file = "./results/manyglm_anova_R.rds")

anova.adj.I <- readRDS("./results/manyglm_anova_I.rds")
anova.adj.S <- readRDS("./results/manyglm_anova_S.rds")
anova.adj.R <- readRDS("./results/manyglm_anova_R.rds")
...
predict.dist <- get_prediction(model = mara.m, new_data = ENV.new, target_variable) 
...
ENV.new <- tibble(
  ...
  Protein_lag1 =   rep(mean(ENV$Protein_lag1),500),
  Height_lag1 =   rep(mean(ENV$Height_lag1),500),
  sin_month =   rep(sin(2*pi*2/12), 500),# set to July, the month with the most counts of cattle dung 
  cos_month =   rep(cos(2*pi*2/12), 500)
)

predict.dist2 <- get_prediction(model = mara.m, new_data = ENV.new, target_variable) 


#species.list <- pull(uni.p.df[which(uni.p.df[uni.p.df$covariate == target_variable,]$p <= 0.05),2])
p.dist2 <- predict.dist2 %>% 
