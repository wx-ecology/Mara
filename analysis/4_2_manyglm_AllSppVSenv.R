...
############################## prep df ########################################
data <- read_csv("./data/for-spp-relationship/mara-cooccurence-compiled.csv") # 1,680 × 40
...
mara.m2 <- manyglm(COUNT ~ Transet + Pgrazed_lag1 + 
                     Precip + Protein_lag1 + Height_lag1 + 
                     sin_month + cos_month, data = ENV, family="negative.binomial")
...
#  saveRDS(anova.adj.I, file = "./results/manyglm_allspp_I.rds")
